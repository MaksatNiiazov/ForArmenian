...
class AdAddress(models.Model):
    ad = models.ForeignKey(Ad, on_delete=models.CASCADE, related_name='addresses')
    ...
    location = models.ForeignKey(Location, on_delete=models.CASCADE, related_name='addresses')
...
class ABCAd(Ad):
    ...
    def get_absolute_url(self):
        return reverse('abc-detail', kwargs={"pk": self.pk})
...
class JobVacancy(JobAd):
    ...
    def get_absolute_url(self):
        return reverse('job-detail-vacancy', kwargs={"pk": self.pk})
...
class JobResume(JobAd):
    ...
    def get_absolute_url(self):
        return reverse('job-detail-resume', kwargs={"pk": self.pk})
...
class EntertainmentAd(Ad):
    ...
    def get_absolute_url(self):
        return reverse('entertainments-detail', kwargs={"pk": self.pk})
...
class CafeAd(Ad):
    type = models.CharField(max_length=20, default='cafe')
    start_time = models.TimeField(blank=True)
    end_time = models.TimeField(blank=True)
...
class CafeKitchen(models.Model):
    cafe = models.ForeignKey(CafeAd, on_delete=models.CASCADE, related_name='kitchens')
    kitchen = models.CharField(max_length=30)


class CafeSpecific(models.Model):
    cafe = models.ForeignKey(CafeAd, on_delete=models.CASCADE, related_name='specifics')
...
class ServiceReview(models.Model):
    service = models.ForeignKey(ServiceAd, on_delete=models.CASCADE, related_name='service_reviews')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='service_reviews')
    review = models.CharField(max_length=500)
    rating = models.SmallIntegerField(validators=[MinValueValidator(1), MaxValueValidator(5)], default=5)
